"""
StreamRank — scraper_starwars.py
Scrapea Top 50 Star Wars desde TMDb API
Guarda en: json/starwars_top50.json
"""
...
GENEROS  = {28:"Acción",12:"Aventura",16:"Animación",35:"Comedia",18:"Drama",14:"Fantasía",878:"Ciencia Ficción"}
...
def fetch_peliculas():
    ...
    for page in range(1, 4):
        params = {"api_key":API_KEY,"language":"es-ES","with_keywords":"161176","sort_by":"vote_average.desc","vote_count.gte":200,"page":page}
        try:
...
def main():
    ...
    for p in raw:
        ...
        limpias.append({"tmdb_id":p["id"],"titulo":p.get("title","Sin título"),"rating":round(p.get("vote_average",0),1),"votos":p.get("vote_count",0),"genero":GENEROS.get(gids[0],"Ciencia Ficción") if gids else "Ciencia Ficción","imagen_url":f"{IMG_BASE}{p['poster_path']}" if p.get("poster_path") else "","fecha_estreno":p.get("release_date",""),"descripcion":p.get("overview","")[:300],"trailer":"","duracion_min":0,"duracion":"—"})
