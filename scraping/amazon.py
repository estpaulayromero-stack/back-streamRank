...
Scrapea Top 50 Amazon Prime desde TMDb API
...
def fetch_peliculas():
    ...
    for page in range(1, 4):
        params = {"api_key":API_KEY,"language":"es-ES","with_watch_providers":"9","watch_region":"US","sort_by":"vote_average.desc","vote_count.gte":500,"page":page}
        try:
            r = requests.get(f"{BASE_URL}/discover/movie", params=params, timeout=10)
            r.raise_for_status()
            peliculas.extend(r.json().get("results",[]))
        except Exception as e: print(f"  Error página {page}: {e}")
    return peliculas

def main():
    print("Scrapeando Amazon Prime...")
    raw = fetch_peliculas()
    ...
    for p in raw:
        ...
        limpias.append({"tmdb_id":p["id"],"titulo":p.get("title","Sin título"),"rating":round(p.get("vote_average",0),1),"votos":p.get("vote_count",0),"genero":GENEROS.get(gids[0],"Drama") if gids else "Drama","imagen_url":f"{IMG_BASE}{p['poster_path']}" if p.get("poster_path") else "","fecha_estreno":p.get("release_date",""),"descripcion":p.get("overview","")[:300],"trailer":"","duracion_min":0,"duracion":"—"})
    ...
    resultado={"categoria":"amazon","nombre":"Amazon Prime","plataforma":"PRIME VIDEO","color":"#00a8e0","descripcion":"Lo mejor de Amazon Prime Video","fecha_actualizacion":datetime.now().strftime("%Y-%m-%d %H:%M:%S"),"total":len(top),"peliculas":top}
