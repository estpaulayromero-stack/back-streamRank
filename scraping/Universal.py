...
def main():
    ...
    for p in raw:
        ...
        limpias.append({"tmdb_id":p["id"],"titulo":p.get("title","Sin título"),"rating":round(p.get("vote_average",0),1),"votos":p.get("vote_count",0),"genero":GENEROS.get(gids[0],"Acción") if gids else "Acción","imagen_url":f"{IMG_BASE}{p['poster_path']}" if p.get("poster_path") else "","fecha_estreno":p.get("release_date",""),"descripcion":p.get("overview","")[:300],"trailer":""})
