"""
StreamRank — scraper_universal.py
Scrapea Top 50 Universal Studios desde TMDb API
Guarda en: json/universal_top50.json
"""
...
GENEROS  = {28:"Acción",12:"Aventura",16:"Animación",35:"Comedia",80:"Crimen",18:"Drama",10751:"Familiar",14:"Fantasía",27:"Terror",878:"Ciencia Ficción",53:"Suspense"}
...
def fetch_peliculas():
    ...
    for page in range(1, 4):
        params = {"api_key":API_KEY,"language":"es-ES","with_companies":"33","sort_by":"vote_average.desc","vote_count.gte":500,"page":page}
        try:
...
def main():
    ...
    for p in raw:
        ...
        limpias.append({"tmdb_id":p["id"],"titulo":p.get("title","Sin título"),"rating":round(p.get("vote_average",0),1),"votos":p.get("vote_count",0),"genero":GENEROS.get(gids[0],"Acción") if gids else "Acción","imagen_url":f"{IMG_BASE}{p['poster_path']}" if p.get("poster_path") else "","fecha_estreno":p.get("release_date",""),"descripcion":p.get("overview","")[:300],"trailer":"","duracion_min":0,"duracion":"—"})
